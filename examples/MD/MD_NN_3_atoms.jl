...
function forcesNN(x::Array{Float64,1})
    ...
    # # is calculated using the formula below:
    # Etot = ham.Eband + 1/2*sum((ham.rhoa-ham.rho).*Vhar)*dx;
    forces = ham.atoms.force[:]

    forces = forces - sum(forces)/3
...
@time (x, v, vdot, E) = time_evolution(velocity_verlet, x -> forcesNN(x), dt, 300, x0, x1)

# figure(1);
# plot( collect(1:size(x)[1])*dt, x )
# title("Trajectory")
# xlabel("time")
# ylabel("position")
