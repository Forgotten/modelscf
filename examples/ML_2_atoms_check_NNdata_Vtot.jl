...
# in this case we study the convergence of the method when we feed the
# total potential produced by the Neural Network to the
# SCF iteration
...
Input_str = string("Input_KS_scf_", Natoms,"_sigma_2.0_Vtot.h5")
# Output_str = string("OutputNN.h5")
Output_str = string("OutputNN_sigma_2.0_Vtot.h5")
Output_str_ref = string("Output_KS_scf_", Natoms,"_sigma_2.0_Vtot.h5")
...
for ii = 1:Nsamples
    ...
    mixOpts = andersonMixOptions(ham.Ns, 0.5, 10 )
    ...
    ham.Vtot = reshape(Output[:,ii], Ns,1)
    println("Error between Vtot and Vtot_NN ", norm( Output[:,ii] -
                                                   OutputRef[:,ii])/
                                                   norm(OutputRef[:,ii]))

    update_psi!(ham, eigOpts);
    update_rho!(ham,Nocc);
    rhoNN = deepcopy(ham.rho)
